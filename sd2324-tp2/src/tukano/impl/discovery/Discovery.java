...
class DiscoveryImpl implements Discovery {
	...
	Map<URI, Instant> urisTimestamp = new ConcurrentHashMap<>();
	...
	public URI[] knownUrisOf(String serviceName, int minEntries) {
		while(true) {
			if(serviceName.equals(Blobs.NAME)){
				...
				Set<URI> aliveBlobs = new HashSet<>(urisTimestamp.keySet());
				
				uris.put(Blobs.NAME, aliveBlobs);
			}
				...
				for (;;) {
					try {
						...
						if (parts.length == 2) {
							...
							if(serviceName.equals(Blobs.NAME)){
								urisTimestamp.remove(uri);
								urisTimestamp.put(uri, Instant.now());
							}
						}
	...
	private void removeInactiveUris(){

		new Thread(() -> {
			while(true){
				for(var entry : urisTimestamp.entrySet()){
					if(Instant.now().minusSeconds(10).isAfter(entry.getValue())){
						urisTimestamp.remove(entry.getKey());
					}
				}
			}
		}).start();
