...
public class JavaShorts implements ExtendedShorts {
	...
	public Result<Short> getShort(String shortId) {
		...
		Result<Short> res = shortFromCache(shortId);
		if (!res.isOK())
			return error(NOT_FOUND);

		Short shrt = res.value();
		...
		return errorOrResult(getShort(shortId), shrt -> {
			...
			return errorOrResult(okUser(shrt.getOwnerId(), password), user -> {
				return DB.transaction(hibernate -> {
					...
					var blobUrls = shrt.getBlobUrl().split("|");
					for (String blobUrl : blobUrls) {
						try {
							String baseUrl = blobUrl.split("/" + Blobs.NAME + "/")[0];
							URI uri = URI.create(baseUrl);
							BlobsClients.get(uri).delete(blobUrl, Token.get());
						} catch (Exception e) {
							e.printStackTrace();
	...
	private String getLiveBlobsUrl(String shortId) {
		...
		return Arrays.stream(knownUris)
        .map(uri -> String.format("%s/%s/%s", uri.toString(), Blobs.NAME, shortId))
