...
public class RestShortsClient extends RestClient implements ExtendedShorts {
	...
	public Result<Short> _createShort(String userId, String password) {
		return super.toJavaResult(target.path(userId).queryParam(RestShorts.PWD, password).request()
				.accept(MediaType.APPLICATION_JSON).post(Entity.json(null)), Short.class);
	}
	...
	public Result<Short> _getShort(String shortId) {
		return super.toJavaResult(target.path(shortId).request().get(), Short.class);
	}
	...
	public Result<Void> _deleteAllShorts(String userId, String password, String token) {
		return super.toJavaResult(
				target.path(userId).path(RestShorts.SHORTS).queryParam(RestExtendedShorts.PWD, password)
						.queryParam(RestExtendedShorts.TOKEN, token).request().delete());
	}
